...
def load_q_table():
    """ ✅ ตรวจสอบว่า Q-Table โหลดได้จริงหรือไม่ """
    global q_table
    if os.path.exists(Q_TABLE_FILE):
        with open(Q_TABLE_FILE, "r") as f:
            try:
                ...
                print(f"✅ Q-Table Loaded: {len(q_table)} states, {total_actions} total actions")
            except json.JSONDecodeError:
                print("⚠️ WARNING: โหลด Q-Table ไม่สำเร็จ! อาจเกิดจากไฟล์เสียหาย")
                q_table = {}
    else:
        q_table = {}
        print("⚠️ ไม่มีไฟล์ Q-Table! กำลังสร้าง Q-Table ใหม่...")

def save_q_table():
    """ ✅ บันทึก Q-Table ลงไฟล์ JSON อย่างปลอดภัย ด้วย FileLock """
    with FileLock(LOCK_FILE):  # 🔒 ล็อกไฟล์ก่อนเขียน เพื่อป้องกันการชนกัน
        temp_file = Q_TABLE_FILE + ".tmp"

        # ✅ ลบไฟล์ `.tmp` ก่อนเขียนใหม่ (ถ้ามี)
        ...
        try:
            ...
            # ✅ ใช้ `shutil.move()` เพื่อเลี่ยงปัญหาการล็อกไฟล์
            shutil.move(temp_file, Q_TABLE_FILE)

        except Exception as e:
            print(f"⚠️ ERROR: ไม่สามารถบันทึก Q-Table ได้: {e}")
...
def update_q_table(state, action, reward, next_state):
    ...
    """
    state_key = str(tuple(tuple(row) for row in state))  # ✅ แปลงเป็น `str`
...
def train_ai(episodes, grid):
    ...
    for episode in range(episodes):
        ...
        for _ in range(GRID_ROWS * GRID_COLS):
            action = choose_action(state, step)  # ✅ เพิ่ม step เข้าไป
