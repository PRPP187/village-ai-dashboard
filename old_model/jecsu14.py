...
save_lock = threading.Lock() # ✅ ใช้ Lock ป้องกันไฟล์เสียหายจากการเขียนพร้อมกัน

last_update_time = time.time() # ✅ เก็บเวลาอัปเดตล่าสุด
...
def update_q_table(state, r, c, action, reward, next_grid, next_r, next_c):
    """
    ✅ อัปเดตค่า Q-Table ตาม Q-Learning Algorithm
    ✅ ใช้ Batch Update เพื่อลดการโหลดเซิร์ฟเวอร์
    """
    global last_update_time

    state_key = str(get_state(state, r, c))
    next_state_key = str(get_state(next_grid, next_r, next_c))
    ...
    #print(f"📌 AI จำแพทเทิร์น: {state_key} -> {action_key} | Reward: {reward}")

    # ✅ อัปเดต Q-Table ทุก 5 วินาที
...
def choose_action(grid, step):
    """
    ✅ เลือกตำแหน่งที่ดีที่สุดเพื่อวางอาคาร ('H', 'R', 'G')
    ✅ เลือกจาก 3 อันดับแรกที่ได้คะแนนดีที่สุด
    """
    rows, cols = len(grid), len(grid[0])

    # ✅ หาเป้าหมาย 'E' ในกระดาน
    e_position = [(r, c) for r in range(rows) for c in range(cols) if grid[r][c] == 'E'][0]
    e_row, e_col = e_position

    # ✅ หาตำแหน่งว่างทั้งหมดและเรียงตามระยะทางจาก 'E'
    ...
    # ✅ ทดลองวางอาคารทุกประเภท และคำนวณคะแนน
    ...
    if top_choices:
        ...
        r, c = chosen[1]
        char = chosen[2]
        return r, c, char  # ✅ คืนค่าตำแหน่งและตัวอักษรที่เลือก
    else:
        return None, None, None  # ❌ ไม่มีที่วางให้คืนค่า None

# ✅ ฟังก์ชัน Train AI

def train_ai(episodes, grid):
    """
    ✅ ฝึก AI โดยใช้ Q-Learning Algorithm
    ✅ บันทึก Q-Table ลงไฟล์ทุก 5 วินาที
    """
    best_grid = None
    best_score = float('-inf')

    for episode in range(episodes):
        state = [row[:] for row in grid]  # ✅ สร้างสำเนากระดาน
        total_reward = 0
        step = 0
        ...
        for _ in range(GRID_ROWS * GRID_COLS):
            ...
            if action is None or action[0] is None:  # ✅ ตรวจสอบว่ามีที่วางหรือไม่
                break

            r, c, char = action
            state[r][c] = char  # ✅ วางอาคารลงในกระดาน

            # ✅ คำนวณรางวัล
            reward = calculate_reward_verbose(state)

            # ✅ สร้างสถานะถัดไป (next_grid)
            next_grid = copy.deepcopy(state)
            next_action = choose_action(state, step) if any('0' in row for row in next_grid) else (r, c, char)

            if next_action is None or next_action[0] is None:  # ✅ ตรวจสอบว่ามีที่วางหรือไม่
                next_r, next_c = r, c
            else:
                next_r, next_c, _ = next_action

            # ✅ อัปเดต Q-Table
            update_q_table(state, r, c, action, reward, next_grid, next_r, next_c)
