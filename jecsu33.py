# =========================================================
# 📌 Import Library และกำหนดค่าพื้นฐาน
# =========================================================

# ✅ Import ไลบรารีมาตรฐาน
import os
import sys
import json
import time
...
if __name__ == "__main__":
    ...
    try:
        ...
        best_results = raw_results  # ดึงค่าจาก train_ai_parallel()
        ...
        for grid_size, best_layout_dict in best_results.items():
            if not isinstance(best_layout_dict, dict):
                continue  # ข้ามกรณีที่ผลลัพธ์ผิดพลาด
                
            for e_position, (best_grid, best_score) in best_layout_dict.items():
                print(f"📌 Grid {grid_size}, E at {e_position} | Score: {best_score}")
        ...
        # ✅ แสดงรายการไฟล์สำรองข้อมูลเฉพาะเมื่อมีปัญหา
        if "error" in globals():
            print("\n📁 รายการไฟล์สำรองข้อมูล:")
            backups = list_backups()
            for backup in backups:
                print(f"   - {backup}")
        # ✅ แสดงเฉพาะจำนวนไฟล์สำรองข้อมูล
        backups = list_backups()
        print(f"\n📁 จำนวนไฟล์สำรองข้อมูลที่มีอยู่: {len(backups)} ไฟล์")
